...
export XAI_API_KEY="${apiKey}"`
  ...
  // Shape follows Grok Build user guide (~/.grok/docs + custom-models) and production-ready Mishra Miron API setups.
  ...
  const configContent = `# Grok Build CLI → Mishra Miron API Grok group (API key auth).
# Docs: ~/.grok/docs/user-guide/05-configuration.md + 11-custom-models.md
# Verify after save: grok inspect
#
# IMPORTANT: api_backend must be "responses" for Mishra Miron API Grok (POST /v1/responses).
...
# Prefer API key when using a custom gateway (matches Mishra Miron API).
...
description = "Grok 4.5 via Mishra Miron API (Responses)"
...
# Imagine tools: model IDs go to Mishra Miron API media endpoints (not the text [model.*] catalog).
...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  const configContent = `# Codex CLI → Mishra Miron API Grok group
...
# Grok/Mishra Miron API path is HTTP/SSE; disable WS (Codex may otherwise try WebSocket first)
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  if (platform === 'gemini') {
    ...
    // Custom provider pointing at Mishra Miron API OpenAI-compatible Responses/Chat endpoints.
    provider[platform].npm = '@ai-sdk/openai-compatible'
    provider[platform].name = 'Grok via Mishra Miron API'
