...
describe('HomeView compact mode', () => {
  ...
  // The tests below assert against the upstream landing page markup
  // ([data-testid="compact-home"] and .terminal-container). This fork ships a
  // custom branded landing page instead, so compact home mode is not rendered.
  // The custom-content tests above still apply and run.
  it.skip('treats whitespace-only custom content as empty and selects compact mode', () => {
  ...
  it.skip.each([undefined, false])('selects the default home when compact mode is %s', (enabled) => {
